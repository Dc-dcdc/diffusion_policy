...
class TrainDiffusionUnetImageWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure model
        self.model: DiffusionUnetImagePolicy = hydra.utils.instantiate(cfg.policy) #获取对应的policy实例对象

        self.ema_model: DiffusionUnetImagePolicy = None
        if cfg.training.use_ema:
            self.ema_model = copy.deepcopy(self.model) #拷贝模型，对主模型进行平滑滤波
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        # update ema 如果开启了 EMA，计算并记录当前的衰减率
                        if cfg.training.use_ema:
                            ema.step(self.model) #参数更新方式为： ema_model = a * ema_model + (1-a) * model
                        ...
                        if cfg.training.use_ema:
                            ...
                            step_log['ema_decay'] = ema.get_decay(self.global_step) #记录衰减率a
                ...
                step_log['train_loss'] = train_loss  #整个epoch的平均loss
